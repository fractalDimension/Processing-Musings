...
int GRID_SCALE = 10;              // Scale of grid is 1/24 of screen size

// odd square number window centered on current square
int ODD_NUM = 9;
int NEIGHBORHOOD = ODD_NUM * ODD_NUM;
int HALF_WIN = (ODD_NUM - 1) / 2;
int TREE_DEPTH = 2;
...
void setup() {
  size(640,360);
  //dnasize = (width / GRID_SCALE) * (height / GRID_SCALE);
  COLS = width / GRID_SCALE;
  ROWS = height / GRID_SCALE;
  lifetime = width/3;
