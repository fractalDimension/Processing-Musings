...
class Population {
  ...
  Population(float m, int num) {
    ...
    for (int i = 0; i < population.length; i++) {
      ...
      population[i] = new Rocket(position, new DNA(TREE_DEPTH));
    }
  ...
  void live (ArrayList<Obstacle> o) {
    ...
    if (debug) {
      population[closest].debugDraw();
      /*
      int gen = getGenerations();
      if (gen > CURRENT_GEN) {
        CURRENT_GEN++;
        println("current gen: ", CURRENT_GEN);
        float[] bestGenes = population[closest].dna.genes;
        for (int i = 1; i < bestGenes.length; i++) {
          if (i % ODD_NUM != 0) {
            print(bestGenes[i-1]+",");
          } else {
            println(bestGenes[i-1]);
          }
        }
        
        
        
        println(bestGenes[0],bestGenes[1],bestGenes[2]);
        println(bestGenes[3],bestGenes[4],bestGenes[5]);
        println(bestGenes[6],bestGenes[7],bestGenes[8]);
  ...
  void generate() {
    ...
    for (int i = 0; i < population.length; i++) {
      ...
      PVector position = new PVector(start.r.x+start.r.width/2, start.r.y+start.r.height/2);
      population[i] = new Rocket(position, childGenes);
    }
    generations++;
  }
  ...
  float getTotalFitness() {
    ...
    for (int i = 0; i < population.length; i++) {
      ...
      totalGeneLength += population[i].dna.genes.size();
      total += currentfitness;
    }
    
    println("--------------------------- Fittest ------------------------------");
    population[bestFitIndex].dna.genes.get(0).printTree(" ", 0);
    println("Average gene length:", totalGeneLength / population.length);
